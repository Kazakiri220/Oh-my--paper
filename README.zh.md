...
重启 Claude Code，在你的 MPAcc 论文项目里运行 `/omp:setup`，然后用 `/omp:survey`、`/omp:ideate`、`/omp:experiment`、`/omp:write` 驱动完整论文流程。不需要 GUI，不需要切窗口，所有事情都在终端里完成。
...
如果你的环境里 `codex` 不在 `PATH` 上，脚本仍会先把插件注册进去，然后提示你去 Codex 的 Plugins 页面完成最后一步。若需要搜索，优先搜 `Oh My Paper - mpacc-thesis-specialized` 或 `oh-my-paper-codex`，不要只搜 `omp`。
...
- 直接自然语言描述，例如：`Use Oh My Paper - mpacc-thesis-specialized to initialize this MPAcc thesis project and scaffold .pipeline/`
