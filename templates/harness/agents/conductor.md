...
- 通过 delegate 派遣 Codex 执行材料整理、证据核查、章节草拟、引用审查等子任务。
...
| 子命令 | 触发更新的时机 |
|--------|-------------|
| delegate | Codex 返回结果、用户选“接受结果”后 |
| experiment-loop | 用户确认证据矩阵或方法适配结论后 |
| survey-blitz | 选题标准、案例证据、文献材料已写入 |
| paper-sprint | 某交付物或章节写完，用户确认内容后 |
| review-gate | review_log 产出后 |
